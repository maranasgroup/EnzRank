...
def parse_data(act_dir, subs_dir, protein_dir, with_label=True,
               prot_len=2500, prot_vec="Convolution",
               subs_vec="Convolution", subs_len=2048):

    print("Parsing {0} , {1}, {2} with length {3}, type {4}".format(*[act_dir ,subs_dir, protein_dir, prot_len, prot_vec]))

    protein_col = "Protein_ID"
    subs_col = "Compound_ID"
    col_names = [protein_col, subs_col]
    if with_label: 
        label_col = "Label"
        col_names += [label_col]
    act_df = pd.read_csv(act_dir)
    subs_df = pd.read_csv(subs_dir, index_col="Compound_ID")
    ...
    act_df = pd.merge(act_df, subs_df, left_on=subs_col, right_index=True)
    subs_feature = np.stack(act_df[subs_vec].map(lambda fp: fp.split("\t")))
    if prot_vec=="Convolution":
        protein_feature = sequence.pad_sequences(act_df["encoded_sequence"].values, prot_len)
    else:
    ...
    if with_label:
        ...
        return {"protein_feature": protein_feature, "subs_feature": subs_feature, "label": label}
    else:
        return {"protein_feature": protein_feature, "subs_feature": subs_feature}


class subs_Target_Prediction(object):
    ...
    def modelv(self, dropout, subs_layers, protein_strides, filters, fc_layers, prot_vec=False, prot_len=2500,
               activation='relu', protein_layers=None, initializer="glorot_normal", subs_len=2048, subs_vec="ECFP4"):
        ...
        if subs_layers is not None:
            subs_layers = return_tuple(subs_layers)
            for layer_size in subs_layers:
        ...
        #input_dim = filters*len(protein_strides) + subs_layers[-1]
    ...
    def __init__(self, dropout=0.2, subs_layers=(1024,512), protein_strides = (10,15,20,25), filters=64,
                 learning_rate=1e-3, decay=0.0, fc_layers=None, prot_vec=None, prot_len=2500, activation="relu",
                 subs_len=2048, subs_vec="ECFP4", protein_layers=None):
        self.__dropout = dropout
        self.__subss_layer = subs_layers
        ...
        self.__subs_vec = subs_vec
        self.__subs_len = subs_len
        self.__activation = activation
        self.__protein_layers = protein_layers
        self.__decay = decay
        self.__model_t = self.modelv(self.__dropout, self.__subss_layer, self.__protein_strides,
                                     self.__filters, self.__fc_layers, prot_vec=self.__prot_vec,
                                     prot_len=self.__prot_len, activation=self.__activation,
                                     protein_layers=self.__protein_layers, subs_vec=self.__subs_vec,
                                     subs_len=self.__subs_len)
...
#         K.get_session().run(tf.initialize_all_variables())
    ...
    def fit(self, subs_feature, protein_feature, label, n_epoch=10, batch_size=32):
        for _ in range(n_epoch):
            history = self.__model_t.fit([subs_feature,protein_feature],label, epochs=_+1, batch_size=batch_size, shuffle=True, verbose=1,initial_epoch=_)
        return self.__model_t
    
    def summary(self):
        self.__model_t.summary()
    
    def validation(self, subs_feature, protein_feature, label, output_file=None, n_epoch=10, batch_size=32, **kwargs):

        if output_file:
            param_tuple = pd.MultiIndex.from_tuples([("parameter", param) for param in ["window_sizes", "subs_layers", "fc_layers", "learning_rate"]])
            result_df = pd.DataFrame(data = [[self.__protein_strides, self.__subss_layer, self.__fc_layers, self.__learning_rate]]*n_epoch, columns=param_tuple)
            result_df["epoch"] = range(1,n_epoch+1)
        result_dic = {dataset: {"AUC":[], "AUPR": [], "opt_threshold(AUPR)":[], "opt_threshold(AUC)":[] }for dataset in kwargs}

        for _ in range(n_epoch):
            history = self.__model_t.fit([subs_feature,protein_feature],label,
            ...
            for dataset in kwargs:
                ...
                test_d = kwargs[dataset]["subs_feature"]
...
if __name__ == '__main__':
    ...
    This Python script is used to train, validate, test deep learning model for prediction of subs-target interaction (act)\n
    ...
    act, subs and protein data must be written as csv file format. And feature should be tab-delimited format for script to parse data.\n
    ...
    """)
    # train_params
    parser.add_argument("act_dir", help="Training act information [subs, target, label]")
    parser.add_argument("subs_dir", help="Training subs information [subs, SMILES,[feature_name, ..]]")
    ...
    parser.add_argument("--test-act-dir", "-i", help="Test act [subs, target, [label]]", nargs="*")
    parser.add_argument("--test-subs-dir", "-d", help="Test subs information [subs, SMILES,[feature_name, ..]]", nargs="*")
    ...
    parser.add_argument("--subs-layers", '-c', help="Dense layers for subss", default=None, nargs="*", type=int)
    parser.add_argument("--fc-layers", '-f', help="Dense layers for concatenated layers of subs and target layer", default=None, nargs="*", type=int)
    # training_params
    ...
    parser.add_argument("--subs-vec", "-V", help="Type of subs feature", type=str, default="morgan_fp")
    parser.add_argument("--subs-len", "-L", help="subs vector length", default=2048, type=int)
    ...
    test_subss = args.test_subs_dir
    test_sets = zip(test_names, tests, test_subss, test_proteins)
    output_file = args.output
    # model_structure variables
    subs_layers = args.subs_layers
    ...
    type_params = {
        ...
        "subs_vec": args.subs_vec,
        "subs_len": args.subs_len,
    }
    # model parameter
    model_params = {
        "subs_layers": subs_layers,
    ...
    act_prediction_model = subs_Target_Prediction(**model_params)
    ...
    test_dic = {test_name: parse_data(test_act, test_subs, test_protein, with_label=True, **type_params)
                for test_name, test_act, test_subs, test_protein in test_sets}
